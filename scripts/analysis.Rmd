...
df3$location1 <- str_c(df3$addresse, ', ', df3$Postnummer)
...
```{r}
df_gym <- read.csv('data/preprocessed/GYM_locations.csv')
df_eud <- read.csv('data/preprocessed/EUD_locations.csv')
```
...
ggplot(data = NULL, aes(group = Education)) +
  ...
  geom_point(data=df, 
             aes(y=Mun_main_percent*20, x=Kommune, group=Education), position = position_dodge(width = 0.9), shape=3, size=4, show.legend=FALSE) +
